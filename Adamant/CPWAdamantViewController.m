...
- (void)initFakeData;
- (IBAction)rHButton:(id)sender;
- (IBAction)pressureButton:(id)sender;
- (IBAction)aTempButton:(id)sender;
- (IBAction)irTempButton:(id)sender;
- (IBAction)viewSavedData:(id)sender;
...
- (void)viewDidLoad
{
    ...
    self.label1.text = [NSString stringWithFormat:@"-%rH"];
    self.label2.text = [NSString stringWithFormat:@"1000mBar"];
    self.label3.text = [NSString stringWithFormat:@"-.-°C"];
    self.label4.text = [NSString stringWithFormat:@"-.-°C"];
...
- (IBAction)rHButton:(id)sender
{
    ...
    button.backgroundColor = [UIColor colorWithRed:0.0/255.0 green:84.0/255.0 blue:109.0/255.0 alpha:1];
    JBLineChartViewController *LCcontroller = [self.storyboard instantiateViewControllerWithIdentifier:@"ChartView"];
    LCcontroller.d = d;
    LCcontroller.sensorType = @"rH";
    [self.navigationController pushViewController:LCcontroller animated:YES];
...
- (IBAction)pressureButton:(id)sender
{
    ...
    JBLineChartViewController *LCcontroller = [self.storyboard instantiateViewControllerWithIdentifier:@"ChartView"];
    LCcontroller.d = d;
    LCcontroller.sensorType = @"pressure";
    [self.navigationController pushViewController:LCcontroller animated:YES];
    //[self performSegueWithIdentifier:@"ChartView" sender:self];
...
- (IBAction)irTempButton:(id)sender
{
    ...
    JBLineChartViewController *LCcontroller = [self.storyboard instantiateViewControllerWithIdentifier:@"ChartView"];
    LCcontroller.d = d;
    LCcontroller.sensorType = @"irTemp";
    [self.navigationController pushViewController:LCcontroller animated:YES];
    //[self performSegueWithIdentifier:@"ChartView" sender:self];
...
-(void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:[self.d.setupData valueForKey:@"IR temperature data UUID"]]]) {
        ...
        self.label3.text = [NSString stringWithFormat:@"%.1f°C",tAmb];
        self.label4.text = [NSString stringWithFormat:@"%.1f°C",tObj];
...
//        self.acc.accGraphZ.progress = (z / [sensorKXTJ9 getRange]) + 0.5;
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:[self.d.setupData valueForKey:@"Humidity data UUID"]]]) {
        ...
        self.label1.text = [NSString stringWithFormat:@"%0.1f%%rH",rHVal];
...
//        self.mag.accGraphZ.progress = (z / [sensorMAG3110 getRange]) + 0.5;
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:[self.d.setupData valueForKey:@"Barometer data UUID"]]]) {
        ...
        self.label2.text = [NSString stringWithFormat:@"%d mBar",pressure];
